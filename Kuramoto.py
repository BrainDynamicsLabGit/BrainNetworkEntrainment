...
class Kuramoto:
    ...
    def load_struct_connectivity(self):
        ...
        if n>90:
            ...
            n=90
        C=C[-n:,-n:]
        C[np.diag(np.ones(n))==0] /= C[np.diag(np.ones(n))==0].mean() ## Normalize such that the mean of all non-diagonal elements is 1.
    ...
    def IntegrateDD(self):
        ...
        print('Simulating %d nodes network using K=%.3f and MD=%.3f at f=%.3fHz'%(self.n_nodes,self.K,self.mean_delay,self.StimFreq/(2*np.pi)))
