...
def mejorar_match(dic_simulado, dic_eeg, labels, cutoff=0.8):
...
def seed_correspondence(
    ...
    line. amplitud_por_frecuencia maps each stimulation frequency (Hz) to the
    corresponding selected forcing amplitude. name_npy_template is formatted
    with the fields {frequency}, {amplitude}, and {seed} to locate each
    simulation file.

    indices_excluidos contains the zero-based indices of subcortical regions
    not represented in the EEG map. These regions are excluded from the
    comparison.
    ...
    For every frequency, Pearson correlation, Spearman correlation, and Dice
    coefficient are calculated for each initial phase configuration. The
    summary reports the mean across configurations and its percentile bootstrap
    95% confidence interval using 10,000 resamples by default.

    Returns the workbook path and four result DataFrames.
    """
    ...
    INDICES_EXCLUIDOS = set(indices_excluidos)
    n_regiones = n_nodes - len(INDICES_EXCLUIDOS)
    ...
    labels = [region for i, region in enumerate(labels90) if i not in INDICES_EXCLUIDOS]

    if len(labels) != n_regiones:
        raise ValueError(f'Unexpected number of comparable regions: {len(labels)}.')
    ...
    for nombre in tabla_activadas.iloc[:, 0].dropna():
        nombre_norm = normalizar_nombre(nombre)
        if nombre_norm in labels:
            regiones_eeg_activadas.add(nombre_norm)
        else:
            match = difflib.get_close_matches(nombre_norm, labels, n=1, cutoff=CUTOFF_MATCH)
            if match:
    ...
    for fstim in FRECUENCIAS:
        ...
        print('=' * 70)
        for seed in SEEDS:
            nombre_npy = name_npy_template.format(
                frequency=fstim,
                amplitude=amplitud,
                seed=seed,
            )
            ...
            sim, eeg = mejorar_match(dicc_simulado, promedios_eeg, labels, cutoff=CUTOFF_MATCH)
            vector_sim = np.array([sim[region] for region in labels], dtype=float)
            vector_eeg = np.array([eeg[region] for region in labels], dtype=float)
            if np.std(vector_sim) == 0 or np.std(vector_eeg) == 0:
                pearson_r = np.nan
                pearson_p = np.nan
            else:
                pearson_r, pearson_p = stats.pearsonr(vector_sim, vector_eeg)
            if np.std(vector_sim) == 0 or np.std(vector_eeg) == 0:
                spearman_rho = np.nan
                spearman_p = np.nan
            else:
                spearman_rho, spearman_p = stats.spearmanr(vector_sim, vector_eeg)
            regiones_simuladas_activadas = {region for region in labels if dicc_simulado[region] > UMBRAL_SIMULADO}
            binarias = metricas_binarias(regiones_eeg_activadas, regiones_simuladas_activadas, labels)
            ...
            for region in labels:
                ...
                detalle_regiones.append({'seed': int(seed), 'frecuencia_hz': float(fstim), 'amplitud': float(amplitud), 'region': region, 'valor_eeg': float(eeg[region]), 'valor_simulado': float(sim[region]), 'eeg_activa': eeg_activa, 'simulada_activa': simulada_activa, 'TP': tp_region, 'FP': fp_region, 'FN': fn_region, 'TN': tn_region})
    ...
    with pd.ExcelWriter(ruta_salida) as writer:
        ...
        df_detalle.to_excel(writer, sheet_name='regiones', index=False)

    return ruta_salida, df_resultados, df_resumen, df_resumen_wide, df_detalle